...
//			moneyEntrepot.classList.remove('allHidden');
		...
		// 精灵上下浮动
			...
			floatimg();
			function floatimg(){
			    if(y1>h||y1<80 && y2>h||y2<80 && y3>h||y3<100){
			    	ySpeed=-ySpeed;
			    }
			    y1+=ySpeed;
			    y2+=ySpeed;
...
//			hhhhhh();
			...
			shopCover.innerHTML = `<div class="shop-covBox">
										...
										<div class="shop-covCont">
											<div class="shop-covCont-t clear">
												...
												<dl>
													<dt>${obj.name}</dt>
													<dd>已拥有<span id='shop-num'>${num.num}</span>件</dd>
												</dl>
											</div>
											...
											<div class="shop-covCont-b">
												...
												<div id='s-right' class="right"><img src="img/shop1-mny.png" width="30px"/><span>${obj.price}</span></div>
											</div>
            ...
            //换换换 按钮
            var shopCovBtn = document.querySelector('.shop-covBtn');
            var shopNum = document.getElementById('shop-num');
            var sRight = document.getElementById('s-right');   //弹窗金额
            
			shopCovBtn.onclick = function(){
				
				if(sRight.innerText < shopSpan.innerText){
					console.log(num)
					num.num++;
					shopNum.innerText = num.num;
					shopSpan.innerText = shopSpan.innerText - sRight.innerText; 
				}
